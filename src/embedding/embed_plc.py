...
MONGODB_TIERS = {
    "dev": (MONGO_URI_PLC, MONGO_DB_NAME_PLC, MONGO_COLL_NAME_PLC),
    # "stg": (MONGO_URI_PLC_STG, MONGO_DB_NAME_PLC_STG, MONGO_COLL_NAME_PLC),
    # "prd": (MONGO_URI_PLC_PRD, MONGO_DB_NAME_PLC_PRD, MONGO_COLL_NAME_PLC),
}
...
def chunk_docs() -> List[Document]:
    docs = []
    sources = []
    df = pd.read_csv(osp.join(PROCESSED_CSV_DIRECTORY, "scraped_plc.csv"))
    ...
    for metadata_str, content_list in grouped.items():
        try:
            ...
            chunks = chunk_page(combined_content, external_metadata, translate=False)
            if chunks:
                docs.extend(chunks)
...
doc_chunks, sources = chunk_docs()

if doc_chunks:
    chunk_path = os.path.join(PROCESSED_CSV_DIRECTORY, "plc_chunks.csv")
    with open(chunk_path, "w", encoding="utf-8", newline="") as new_file:
